...
3. 语言专业、克制、可信，不使用标题党、口号式表达或空泛套话。
4. 如果原文是英文，请准确理解后再输出自然中文。
5. 如果素材信息密度高，必须充分展开，不要压缩成短摘要。
6. 不要在标题、导语、正文或结尾中保留原平台字样和说明性尾巴，例如“文章来源”“原文链接”“本文来自”“免责声明”“据某平台报道”等；除非该平台名称本身就是新闻事实主体。
...
function sanitizeRewriteTitle(title = '', sourceName = '') {
  let output = String(title || '').trim();
  const normalizedSourceName = String(sourceName || '').trim();

  if (normalizedSourceName) {
    const sourcePattern = escapeRegExp(normalizedSourceName);
    output = output
      .replace(new RegExp(`\\s*[-|｜丨]\\s*${sourcePattern}$`, 'i'), '')
      .replace(new RegExp(`^${sourcePattern}\\s*[:：-]\\s*`, 'i'), '')
      .trim();
  }

  return output
    .replace(/^(原标题|原题|来源)[:：]\s*/i, '')
    .trim();
}

function sanitizeRewriteContent(content = '', sourceName = '') {
  const normalizedSourceName = String(sourceName || '').trim();
  ...
  const removableLinePatterns = [
    /^\s*(原文链接|原标题|文章来源|来源|参考链接)[:：].*$/gim,
    /^\s*(本文转载自|本文来源于|本文来自|本文作者|作者[:：]|免责声明|版权声明).*$/gim,
    /^\s*以上内容仅供参考.*$/gim,
    /^\s*更多信息请参阅.*$/gim
  ];

  for (const pattern of removableLinePatterns) {
  ...
  if (normalizedSourceName) {
    ...
    const leadingPatterns = [
      new RegExp(`(^|\\n)\\s*据${sourcePattern}(?:报道|消息|介绍|披露|显示|获悉)?[，,:：]\\s*`, 'g'),
      new RegExp(`(^|\\n)\\s*${sourcePattern}(?:报道|消息|获悉|显示)?[，,:：]\\s*`, 'g'),
      new RegExp(`(^|\\n)\\s*来源[：:]\\s*${sourcePattern}\\s*`, 'g')
    ];
...
4. 不要保留原平台字样和说明性尾巴，例如“文章来源”“原文链接”“本文来自”“免责声明”“据某平台报道”等；除非该平台名称本身就是新闻事实主体。`;
}
...
原文链接：{{link}}`, variables);
  ...
  return {
    title: sanitizeRewriteTitle(parsed.title, options.sourceName),
    content: sanitizeRewriteContent(parsed.content, options.sourceName),
...
5. 输出格式为“标题：”“正文：”。`;
  ...
  return {
    title: sanitizeRewriteTitle(parsed.title),
    content: sanitizeRewriteContent(parsed.content),
...
module.exports = {
  ...
  generateSummary,
  callLLM,
  sanitizeRewriteTitle,
  sanitizeRewriteContent
};
